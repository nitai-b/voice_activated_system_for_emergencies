...
#use rs232(baud=9600,parity=N,xmit=PIN_C4,rcv=PIN_D3,stream=GSM,bits=8)		//SW serial pins Fona	||	GSM
...
void sleep_setup(){					//put device to sleep and wake on double clap or received a character
	char _rec_sleep = NUL;
	...
	if (rec_WS1 == STS_SIMILAR){
		...
		switch (cmd_WS1){
			...
			case WS1_STOP:
				...
				break;
			case WS1_HELLO:
					playback(SND_please_say_a_command); // responds to the user
					event_WS1 = E_LIS_GP1;
				break;
			default:	
					event_WS1 = E_LIS_GP1;
...
int listenGP3(){	//listens to group3 and determines which word was spoken
	...
	if (rec_GP3 == STS_SIMILAR){
		...
		switch (cmd_GP3){
			case G3_ZERO:
				number = 0x30; // ascii rep of 0
				break;
			case G3_ONE:
				number = 0x31; // ascii rep of 1
				break;
			case G3_TWO:
				number = 0x32; // ascii rep of 2
				break;
			case G3_THREE:
				number = 0x33; // ascii rep of 3
				break;
			case G3_FOUR:
				number = 0x34; // ascii rep of 4
				break;
			case G3_FIVE:
				number = 0x35; // ascii rep of 5
				break;
			case G3_SIX:
				number = 0x36; // ascii rep of 6
				break;
			case G3_SEVEN:
				number = 0x37; // ascii rep of 7
				break;
			case G3_EIGHT:
				number = 0x38; // ascii rep of 8
				break;
			case G3_NINE:
				number = 0x39; // ascii rep of 9
				break;
			// case G3_TEN:
				// number = 10;
				// break;
...
void button2(void){
	int16 i = 0;
	// disable_interrupts(INT_TIMER0);// just incase
...
void pulse(void){
	...
	for (i;i<1000;i++)delay_cycles(255);
	output_low(PIN_D0);
...
// This just for verifying that the VR module works.
