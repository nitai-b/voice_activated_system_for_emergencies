...
enum group1{					//constants for action words
	G1_CALL, G1_ANSWER, G1_LIGHTS, G1_LED, G1_OTHER
};
...
volatile char rec_string[16]; //string of size 16
#INT_EXT HIGH
void EXT_isr(void){
	fgets(rec_string, GSM);
	if(strncmp(rec_string,OK,strlen(OK))==0){
		call_flag = TRUE;
		current_state = S_RECEIVE_CALL;
	}
}
