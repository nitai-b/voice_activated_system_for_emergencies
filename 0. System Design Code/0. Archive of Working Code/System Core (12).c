...
#use rs232(baud=9600,parity=N,xmit=PIN_D0,rcv=PIN_D1,stream=GSM,bits=8)		//SW serial pins Fona	||	GSM
...
////Some Constants and Setup
	gsmSetup();
	char c,k; //a variable for testing purposes
	
////setup interrupts
	// set_tris_b(0x01);
	// ext_int_edge(H_TO_L);			//interrupt on falling edge of pin RB0
	// clear_interrupt(INT_EXT);		//clear external interrupt
	...
	wake_on_whistle();			//sets to wake on whistle
////Program Loop
	while (1){
		fprintf(PC,"\r\n");
...
//************Setup*************
	...
	setTimeout(ARG_FIVE);		//set the timeout for 5 seconds
	setLang(ARG_ZERO);			//set the language to english
	sleep&wake();				//sets to wake on whistle
	gsmSetup();
