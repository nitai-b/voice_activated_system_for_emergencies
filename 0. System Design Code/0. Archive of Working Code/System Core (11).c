...
#define BUFF_SIZE 32
volatile unsigned int got_string_flag;
volatile unsigned char received_char;
volatile unsigned char rec_buff[BUFF_SIZE];
volatile unsigned int rx_isr_index;
...
void serial_rx_isr(void){
	...
	disable_interrupts(INT_RDA);
		if (rx_isr_index == BUFF_SIZE) rx_isr_index = 0;
		rec_buff[rx_isr_index] = getc();
		if (rec_buff[rx_isr_index] == '\r'){
			got_string_flag = 1;
			rx_isr_index = 0;
		}
		rx_isr_index++;
